...
///
/// ```no_run
/// use agenomic_contract::{evaluate_contract, BehaviorContract, TraceEnvelope};
...
///       required_fields: [foo]
/// "#;
/// let c = agenomic_contract::parse_contract_yaml(yaml).unwrap();
/// assert_eq!(c.id, "c1");
/// ```
pub fn parse_contract_yaml(yaml_text: &str) -> CliResult<BehaviorContract> {
    let v: serde_yaml::Value = serde_yaml::from_str(yaml_text)
        .map_err(|e| agenomic_core::CliError::Schema(format!("yaml: {e}")))?;
    let c = v
        .get("contract")
        .cloned()
        .ok_or_else(|| agenomic_core::CliError::Schema("missing 'contract' key".into()))?;
    let parsed: BehaviorContract = serde_yaml::from_value(c)
        .map_err(|e| agenomic_core::CliError::Schema(format!("contract parse: {e}")))?;
    Ok(parsed)
}
...
pub fn parse_traces_jsonl(text: &str) -> CliResult<Vec<TraceEnvelope>> {
    ...
    for (lineno, line) in text.lines().enumerate() {
        ...
        let t: TraceEnvelope = serde_json::from_str(line).map_err(|e| {
            agenomic_core::CliError::Schema(format!("trace line {}: {e}", lineno + 1))
        })?;
        out.push(t);
    }
    Ok(out)
