//! Create and verify AgentLock release attestations.
//!
//! An attestation pins:
//!
...
///
/// ```no_run
/// use agentlock_attestation::{create_attestation, AttestationOptions};
...
pub fn create_attestation(options: AttestationOptions) -> CliResult<ReleaseAttestation> {
    let summary = agentlock_bundle::inspect_bundle(&options.bundle)?;
    ...
    if let Some(SigningMode::LocalEd25519 { key_path }) = &options.sign_with {
        let sk = agentlock_atep::load_signing_key(key_path)?;
        ...
        att.signature = Some(AttestationSignature {
            ...
            key_id: agentlock_atep::short_key_id(&sk.verifying_key()),
    ...
    agentlock_fs::write_atomic(&options.output, &json)?;
...
pub fn verify_attestation(
    ...
    if let (Some(atep_path), Some(expected)) = (atep_store, &att.atep_root_hash) {
        ...
        match std::fs::read(&manifest_path) {
            Ok(b) => match serde_json::from_slice::<agentlock_atep::AtepManifest>(&b) {
...
mod tests {
    ...
    fn signed_create_then_verify_ok() {
        ...
        agentlock_atep::generate_signing_key(&key).unwrap();
