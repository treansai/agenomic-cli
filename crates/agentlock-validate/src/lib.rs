//! Multi-level validation for AgentLock bundle directories and archives.
//!
//! Three levels:
//!
...
use agentlock_spec::{validator, SchemaKind};
...
/// use agentlock_core::ValidationLevel;
...
pub fn validate_bundle(dir: &Path, level: ValidationLevel) -> CliResult<ValidationReport> {
    ...
    for required in REQUIRED_FILES {
        ...
        if !p.is_file() {
            report.push_error(ValidationIssue {
                code: "agentlock::bundle::missing_required_file".into(),
                severity: Severity::High,
                message: format!("missing required file: {required}"),
                path: Some(required.to_string()),
                hint: Some("run `agentlock init` to scaffold the bundle".into()),
                doc: None,
            });
        }
    }
    ...
    if !have_lockfile {
        report.push_error(ValidationIssue {
            ...
            hint: Some("run `agentlock init` to scaffold the lockfile".into()),
            doc: None,
        });
    }
    ...
    if !prompts_present {
        report.push_warning(ValidationIssue {
            code: "agentlock::bundle::missing_prompts".into(),
    ...
    if let Some(t) = &genome_text {
        if let Err(e) = serde_yaml::from_str::<serde_yaml::Value>(t) {
            report.push_error(ValidationIssue {
                code: "agentlock::bundle::yaml_parse".into(),
                severity: Severity::High,
    ...
    if let Some(t) = &lock_text {
        run_schema(SchemaKind::AgentLock, t, "agent.lock.yaml", &mut report)?;
    }
    if let Some(t) = &contract_text {
        run_schema(
    ...
    if let Some(t) = &genome_text {
        if let Ok(v) = serde_yaml::from_str::<serde_yaml::Value>(t) {
            ...
            if fp.is_empty() {
                report.push_error(ValidationIssue {
                    code: "agentlock::ci::missing_model_fingerprint".into(),
                    severity: Severity::High,
            ...
            if let Some(arr) = v.get("knowledge").and_then(|x| x.as_sequence()) {
                for k in arr {
                    ...
                    if snap.is_empty() {
                        report.push_error(ValidationIssue {
                            code: "agentlock::ci::missing_snapshot_hash".into(),
                            severity: Severity::High,
                            message: format!(
...
pub fn validate_archive(archive: &Path, level: ValidationLevel) -> CliResult<ValidationReport> {
    ...
    agentlock_bundle::extract_bundle(agentlock_bundle::ExtractOptions {
...
pub fn validate_agent_lock(lock_yaml: &str) -> CliResult<ValidationReport> {
    let mut report = ValidationReport {
        valid: true,
        ..Default::default()
    };
    run_schema(
        SchemaKind::AgentLock,
        lock_yaml,
        "agent.lock.yaml",
        &mut report,
    )?;
...
pub fn validate_atep_event(event_json: &str) -> CliResult<ValidationReport> {
    ...
    if let Err(errs) = validator.validate(&v) {
        for err in errs {
            report.push_error(ValidationIssue {
                code: "agentlock::atep::schema".into(),
                severity: Severity::High,
...
fn run_schema(
    ...
    if let Err(errs) = v.validate(&json) {
        for err in errs {
            report.push_error(ValidationIssue {
                code: format!("agentlock::schema::{}", kind.label()),
                severity: Severity::High,
                message: err.to_string(),
...
fn cross_reference(
    ...
    if let (Some(g), Some(l)) = (&g_yaml, &l_yaml) {
        ...
        if !g_id.is_empty() && !l_id.is_empty() && g_id != l_id {
            report.push_error(ValidationIssue {
                code: "agentlock::xref::agent_id_mismatch".into(),
                severity: Severity::High,
                message: format!(
        ...
        for t in g_tools.difference(&l_tools) {
            report.push_warning(ValidationIssue {
                code: "agentlock::xref::tool_in_genome_not_locked".into(),
                severity: Severity::Medium,
                message: format!("tool '{t}' declared in genome but not present in lockfile"),
                path: Some("agent.lock.yaml".into()),
                hint: Some("run `agentlock build` to refresh the lockfile".into()),
                doc: None,
            });
        }
        for t in l_tools.difference(&g_tools) {
            report.push_warning(ValidationIssue {
                code: "agentlock::xref::tool_in_lock_not_genome".into(),
                severity: Severity::Medium,
