...
use agentlock_core::{io_at, CliResult, Severity, ValidationIssue};
use walkdir::WalkDir;
    ...
    for dirent in WalkDir::new(dir).follow_links(false) {
        ...
        if rel.split('/').any(|seg| seg == "..") {
            out.push(ValidationIssue {
                code: "agentlock::security::path_traversal".into(),
        ...
        if file_type.is_symlink() {
            out.push(ValidationIssue {
                code: "agentlock::security::symlink".into(),
        ...
        if file_type.is_file() {
            ...
            if let Some(fname) = path.file_name().and_then(|s| s.to_str()) {
                if SECRET_NAMES.contains(&fname) || fname.starts_with(".env.") {
                    out.push(ValidationIssue {
                        code: "agentlock::security::secret_file".into(),
                        severity: Severity::Critical,
                        message: format!("credential file present: {rel}"),
                        path: Some(rel.clone()),
                        hint: Some("delete or .agentlockignore".into()),
                        doc: None,
                    });
                }
            }
            ...
            if let Ok(meta) = dirent.metadata() {
                if meta.len() > DEFAULT_MAX_FILE_BYTES {
                    out.push(ValidationIssue {
                        code: "agentlock::security::oversize_file".into(),
                    ...
                    for (lineno, line) in text.lines().enumerate() {
                        ...
                        for prefix in SECRET_FIELD_PREFIXES {
                            if let Some(rest) = l.strip_prefix(prefix) {
                                ...
                                {
                                    out.push(ValidationIssue {
                                        code: "agentlock::security::inline_secret".into(),
                                        severity: Severity::High,
                                        message: format!(
