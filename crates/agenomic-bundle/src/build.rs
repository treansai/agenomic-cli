...
use agenomic_hash::{compute_manifest_from_pairs, BundleManifest};
...
pub struct BuildBundleOptions {
    ...
    /// If true, fail on validation errors (delegated to `agenomic-validate`
    /// in higher layers).
    pub strict: bool,
...
///
/// ```no_run
/// use agenomic_bundle::{build_bundle, BuildBundleOptions};
...
pub fn build_bundle(options: BuildBundleOptions) -> CliResult<BundleBuildResult> {
    ...
    agenomic_fs::write_atomic(&options.output_file, &compressed)?;
