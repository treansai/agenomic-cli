...
use agenomic_hash::{compute_manifest, compute_manifest_from_pairs};
...
///
/// ```no_run
/// let s = agenomic_bundle::inspect_bundle(std::path::Path::new("./examples/claims-agent")).unwrap();
/// println!("{}", s.agent_id);
/// ```
...
fn collect_yaml_pairs_from_dir(dir: &Path) -> CliResult<Vec<(String, Vec<u8>)>> {
    ...
    for name in &[
        ...
        let p = dir.join(name);
        if p.is_file() {
            let bytes = std::fs::read(&p).map_err(|e| agenomic_core::io_at(&p, e))?;
...
fn summarize(
    pairs: &[(String, Vec<u8>)],
    manifest: &agenomic_hash::BundleManifest,
