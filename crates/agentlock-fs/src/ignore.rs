//! Minimal `.agentlockignore` parser (gitignore-style globs, no negation).

use std::path::Path;

use agentlock_core::{io_at, CliError, CliResult};

/// In-memory representation of a `.agentlockignore` file.
...
impl IgnoreFile {
    ...
    ///
    /// ```no_run
    /// use agentlock_fs::IgnoreFile;
    /// let ig = IgnoreFile::load_from(std::path::Path::new(".agentlockignore")).unwrap();
