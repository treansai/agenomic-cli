//! `agenomic doctor` diagnostic checks.
...
/// let r = agenomic_diagnostics::run_diagnostics(&cfg).await.unwrap();
...
pub async fn run_diagnostics(config: &EffectiveConfig) -> CliResult<DiagnosticReport> {
    ...
    for kind in agenomic_spec::SchemaKind::ALL {
        if agenomic_spec::validator(kind).is_err() {
            schemas_ok = false;
    ...
    checks.push(DiagnosticCheck {
        ...
        detail: Some(if schemas_ok {
            format!("{} schemas parsed", agenomic_spec::SchemaKind::ALL.len())
        } else {
            format!("schema(s) failed: {}", schema_errors.join(","))
        }),
    });

    checks.push(match agenomic_config::config_file_path() {
    ...
    #[cfg(unix)]
    {
        if let Ok(p) = agenomic_config::credentials_file_path() {
            if p.exists() {
    ...
    // BLAKE3 sanity
    let h = blake3::hash(b"agenomic");
    let expected = "agenomic";
    let _ = expected;
    checks.push(DiagnosticCheck {
...
fn atep_smoke_test() -> Result<(), String> {
    ...
    let event = agenomic_atep::AtepEvent::seal(header, payload, &sk, "diag".into())
...
mod tests {
    ...
    async fn smoke_runs() {
        let cfg = EffectiveConfig {
            ...
            profile: agenomic_config::ProfileConfig {
                name: "default".into(),
