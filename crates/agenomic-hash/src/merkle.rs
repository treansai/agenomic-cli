...
//!    order) using [`agenomic_fs::walk_bundle`] with default options.
...
///
/// Walks via [`agenomic_fs::walk_bundle`] with [`WalkOptions::default`],
/// reads each file, hashes it as a Merkle leaf, then builds the tree.
///
/// ```no_run
/// let m = agenomic_hash::compute_manifest(std::path::Path::new("./examples/claims-agent")).unwrap();
    ...
    B: AsRef<[u8]>,
{
    ...
    for (path, content) in pairs {
        ...
        manifest_entries.push(ManifestEntry {
            path: path.clone(),
            kind: agenomic_fs::EntryKind::File,
...
/// ```
/// let bytes = [0u8; 32];
/// assert_eq!(agenomic_hash::format_hash(&bytes, false).len(), 64);
/// assert!(agenomic_hash::format_hash(&bytes, true).starts_with("b3:"));
/// ```
