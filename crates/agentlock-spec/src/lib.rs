//! Embedded JSON Schemas for the AgentLock spec, plus version negotiation.
//!
...
/// Default spec version emitted by `agentlock init`.
...
///
/// ```
/// let s = agentlock_spec::embedded_schema(agentlock_spec::SchemaKind::Genome);
/// assert!(s.contains("AgentLock Genome"));
/// ```
pub fn embedded_schema(kind: SchemaKind) -> &'static str {
    match kind {
        SchemaKind::Genome => GENOME_SCHEMA,
        SchemaKind::AgentLock => AGENT_LOCK_SCHEMA,
...
///
/// ```no_run
/// let _ = agentlock_spec::validator(agentlock_spec::SchemaKind::Genome).unwrap();
/// ```
